...
2. ~~**A test writes to the live experiments directory.**~~ **Resolved before this audit was written:** the auditor flagged `tests/test_bracket.py::test_granularity_bracket_writes_gate_schema` as unfixed from stale SUMMARY/WINDOWS text, but the current code (quick task 260911-7fy, commit `fe82fd4`, 2026-09-11) monkeypatches `config.EXPERIMENTS_DIR` to `tmp_path` — verified at `tests/test_bracket.py:418-430`. No action needed.
