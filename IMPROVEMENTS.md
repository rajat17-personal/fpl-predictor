...
| bracket_ridge | D-15 cheap gate: val played-only Spearman >= LightGBM + 0.010 | 0.3955 vs 0.3900 baseline (+0.0055, need +0.010) | HOLD | off |
| bracket_xgb | D-15 cheap gate: val played-only Spearman >= LightGBM + 0.010 | 0.3949 vs 0.3900 baseline (+0.0049, need +0.010) | HOLD | off |
| bracket_catboost | D-15 cheap gate: val played-only Spearman >= LightGBM + 0.010 | 0.3886 vs 0.3900 baseline (-0.0014, need +0.010) | HOLD | off |
| bracket_mlp | D-15 cheap gate: val played-only Spearman >= LightGBM + 0.010 | 0.3942 (pooled) vs 0.3900 baseline (+0.0042, need +0.010) | HOLD | off |
| bracket_rnn | D-15 cheap gate: single-season played-only Spearman >= LightGBM + 0.010 (Colab-trained, judged against its own 2025-26 test-season baseline, NOT the 6-season bar) | -0.0789 vs 0.3424 same-season baseline (-0.4213, need +0.010) | HOLD | off |
| bracket_transformer | D-15 cheap gate: single-season played-only Spearman >= LightGBM + 0.010 (Colab-trained, judged against its own 2025-26 test-season baseline, NOT the 6-season bar) | 0.0593 vs 0.3424 same-season baseline (-0.2831, need +0.010) | HOLD | off |

Every row started unmeasured; each plan in this phase filled its own row as
it measured it, and plan 10-14 confirms none remain unmeasured — matching
the Phase F table's own closing discipline. D-12's model-class bracket
closes at 7/7 candidates gated, 0 advancing (no candidate cleared
`GATE_MARGIN = 0.010`), so no 6-season walk-forward run was triggered for
any bracket candidate.
...
- **Commands and artifacts, by plan.**
  - Plan 10-10: `python -m models.bracket.gate --candidate lgbm --candidate ridge --candidate xgb --candidate catboost` — writes `data/processed/experiments/bracket_gate_{lgbm,ridge,xgb,catboost}.json`.
  - Plan 10-11: `models.bracket.deep.run_granularity_bracket("mlp")` (both granularity variants) — writes `bracket_gate_mlp.json`, `bracket_search_mlp_per_position.json`, `bracket_search_mlp_pooled.json`.
  - Plan 10-13: the Colab handoff (`models.bracket.registry.export_sequence_bundle` -> `colab/bracket_deep.ipynb` run on Google Colab GPU by the human -> `colab_rnn_2025-26.parquet` / `colab_transformer_2025-26.parquet`), scored locally through `backtest.walk_forward`'s D-14 external-prediction seam (`--external-preds ... --tag bracket_rnn_2526` / `bracket_transformer_2526`, writing `wf_bracket_rnn_2526.json`/`.csv` and `wf_bracket_transformer_2526.json`/`.csv`) — writes `bracket_gate_rnn.json` / `bracket_gate_transformer.json`.
  - Plan 10-14 (this entry): the D-15 mechanical advance rule applied over all seven recorded gate files (`tests/test_bracket.py::test_gate_advance_rule_is_mechanical*`; the live-file comparison verified via a one-off `python -c` read of every `bracket_gate_*.json`, reproduced in the table below) — zero candidates advance, so no 6-season `wf_bracket_<name>_6.json` run exists or was needed.
- **The full seven-row gate comparison table**, verbatim from the mechanical read of the seven recorded `bracket_gate_<candidate>.json` files (train `config.TRAIN_SEASONS` 2016-17..2023-24, val/eval `2024-25` for the four locally-trained candidates; the two Colab candidates are frozen 2025-26-test-season artifacts, see below):

  | candidate | granularity | spearman_xp_med | delta vs its own baseline | mae_xp_med (diagnostic only) | status | verdict |
  |---|---|---:|---:|---:|---|---|
  ...
  | rnn (GRU) | n/a | -0.0789 | -0.4213 | 4.9939 | ok | HOLD |
  | transformer | n/a | 0.0593 | -0.2831 | 2.8074 | ok | HOLD |

  **`eval_split` non-comparability caveat**, stated explicitly per the plan's own requirement: the five locally-trained candidates' figures above (`lgbm`/`ridge`/`xgb`/`catboost`/`mlp`) carry `eval_split: "val_season_in_sample_early_stopping"` — an **in-sample, early-stopping-optimistic** validation-split figure, deliberately used because it is a *relative* comparison where every candidate shares the identical handicap. It is **NOT comparable** to the **0.383** pooled played-only test-season number this file's own Phase F benchmark table records (`backtest.benchmark_external`'s EXP-1 basis). The two Colab candidates (`rnn`/`transformer`) carry `eval_split: "test_season_external_colab_gpu"` instead — a genuine 2025-26 test-season figure, judged against a **freshly recomputed** in-process LightGBM test-season baseline of **0.3424** (not the 0.3900 val-split figure the other five are judged against, and not the 0.383 Phase F number either — three genuinely different baselines coexist in this table by construction of the three different measurement protocols used, and each candidate's HOLD verdict above is correct against its own recorded baseline).
- **6-season walk-forward promotion: not triggered.** The D-15 rule (`spearman_xp_med >= baseline + GATE_MARGIN`, `GATE_MARGIN = 0.010`, `models/bracket/gate.py::_advances`) applied mechanically to all seven rows above yields **zero ADVANCE, seven HOLD**. Per the plan's own instruction, this is the finding — `GATE_MARGIN` was not lowered, no search budget was extended, and no candidate was promoted on a best-of-a-bad-field basis (the `capt_mc` precedent: a gate that does not fire is recorded as a gate that did not fire, IMPROVEMENTS.md above). Consequently **no candidate was run against the >= 2,280 6-season bar** — that bar only applies to a promoted candidate, and none was promoted. `tests/test_bracket.py::test_gate_advance_rule_is_mechanical` and `..._over_the_seven_recorded_candidates` assert this classification directly against both a synthetic table and the live gate files.
- **A locally-trained winner would have run through `scripts/experiment_run.sh bracket_<name>_6 --replicas 5 --experiments bracket_<name>`** (the `stage2` seam `models/bracket/registry.py` exposes) — moot here since no locally-trained candidate (ridge/xgb/catboost/mlp) advanced. The `bracket_ridge`/`bracket_xgb`/`bracket_catboost`/`bracket_mlp` `config.EXPERIMENTS` flags remain registered but are never read by any wiring outside `config.py` itself (verified: `grep -rn "bracket_ridge\|bracket_xgb\|bracket_catboost\|bracket_mlp" --include=*.py` outside `config.py`/`tests/` returns nothing) — this was never exercised as a live inertness risk because the promotion branch that would have called it was never reached.
- **The two Colab candidates' single-season figures are recorded as single-season, never compared to the 6-season 2,280 bar.** Both `rnn` and `transformer` HOLD by a wide margin against their own 2025-26 test-season baseline (deltas -0.4213 and -0.2831 respectively) — this margin is wide enough that no recommendation to spend further Colab budget on a 6-season run is warranted; the single-season reading already settles the question decisively in both directions being negative.
- **D-13 compute accounting.** Declared budget: 200 Colab Pro units total (plus open-ended local WSL overnights for the four non-sequence candidates). Actual consumption: **not reported** — the human-run Colab session for both `rnn` and `transformer` did not surface a units-consumed/remaining figure (10-13-SUMMARY.md), recorded here as "not reported" rather than assumed zero or guessed. Colab-side wall clock was trivial regardless: **RNN 3.3s, transformer 19.5s** GPU training time (mini-batch, batch 8192, ~27 steps/epoch). Local WSL wall clock for the other five: lgbm 8.97s, ridge 8.94s, xgb 13.32s, catboost 13.62s, mlp 100.47s (all at `config.TRAIN_SEASONS`/`config.VAL_SEASON` scope, zero Colab units). No candidate was recorded `not_run_compute_exhausted` — the 200-unit budget was never a binding constraint for this bracket.
- **D-19 budget accounting: configs actually evaluated per deep candidate per granularity, against the declared 12.**
  - `mlp`: **12/12** configs evaluated at the `per_position` granularity (`bracket_search_mlp_per_position.json`, best val loss variant scored 0.3924) and **12/12** at `pooled` (`bracket_search_mlp_pooled.json`, best val loss variant scored 0.3942, the winning granularity) — the full declared budget spent on both variants, hard-stopped at 12 by `models/bracket/deep.py::SEARCH_BUDGET`, not exceeded.
  - `rnn`/`transformer`: **0 of 12** local search configs evaluated. Neither ran through `models/bracket/deep.py::run_search` — D-13's Colab handoff trains one fixed architecture directly on GPU rather than sweeping a local grid. For `transformer` this is intentional and pre-declared: 10-01-PLAN.md's own "Locked planner decisions" table fixed the size budget (`D_MODEL=64, N_HEAD=4, N_LAYERS=2, DIM_FF=128, DROPOUT=0.1`) before this plan sequence began, so no search over transformer size was ever in scope (`models/bracket/transformer.py`'s own module docstring). `models/bracket/recurrent.py` defines a `_GRU_SPACE` bounded at `<= SEARCH_BUDGET (12)` but this candidate's actual Colab run trained the notebook's own single default configuration (`max_epochs` raised 60 -> 200 by the human, `patience` left at the notebook's default 10, tighter than the local shared loop's default of 50) rather than sweeping that space — both deviations from a full local search are recorded here as run-record facts, not corrected after the fact, since the human ran the notebook as delivered.
  - `ridge`/`xgb`/`catboost`: **0/12**, by design — D-19 explicitly declares these get "sensible LightGBM-adjacent defaults only" (asymmetric treatment, not an oversight); `models/bracket/classical.py::RidgeRegressorFactory` and `models/bracket/gbdt.py`'s two factories ignore the incoming LightGBM params dict entirely and use fixed `_RIDGE_PARAMS`/`_XGB_PARAMS`/`_CAT_PARAMS`.
- **GRU chosen over LSTM (D-12's "exactly one recurrent, LSTM or GRU, not both"), recorded as a decision, not an unexplained absence.** Per `models/bracket/recurrent.py`'s own module docstring: GRU has fewer parameters than an LSTM for the same hidden size and sequence length, which matters directly against D-13's fixed 200-unit Colab compute budget; and the 10-gameweek window (`models/bracket/sequence.py::SEQ_WINDOW = 10`) is short enough that LSTM's extra input/forget/output gating buys little over GRU's simpler update/reset gates. No LSTM candidate was ever built or measured — this was a pre-implementation choice, not a comparison that favoured GRU.
- **The mechanical verdict per candidate, and the resulting default.** Every one of the seven candidates **HOLD**s; per D-08, `config.EXPERIMENTS['bracket_ridge']`, `['bracket_xgb']`, `['bracket_catboost']`, `['bracket_mlp']`, `['bracket_rnn']`, `['bracket_transformer']` all stay `False` (already the default; no flip in this phase). All code (`models/bracket/registry.py`, `classical.py`, `gbdt.py`, `deep.py`, `recurrent.py`, `transformer.py`, `gate.py`, the Colab handoff notebook) stays merged and inspectable — nothing deleted. LightGBM remains the shipped stage-2 regressor for every position.
- **D-12's model-class question is closed with numbers for all six challenger families plus the LightGBM control, regardless of the answer:** a linear baseline (ridge), two tree-boosting alternatives (xgb, catboost), a tabular MLP, and two sequence models (GRU, transformer) were all measured against the shipped LightGBM stage-2 regressor on the same feature set, and none moved the ranking metric enough to justify a full 6-season retrain.

### news_sentiment: adoption verdict (plan 10-12)

- **Not built — declined on cost, not not-triggered.** This is the same outcome plan 10-12 itself already recorded under "D-02 news-sentiment: declined on cost, NOT not-triggered (plan 10-12)" above; this entry is the plan-10-14-required ledger heading for that same outcome, written from 10-12-SUMMARY.md's handed-over figures with nothing re-derived.
- **The D-02 trigger fired.** Measured post-Tier-1 pooled played-only `spearman_xp_med` = **0.3874** (< the locked **0.500** threshold, `data/processed/experiments/benchmark_tier1.json`) — the mechanical outcome was BUILD, not "not triggered." A future revisit must be able to tell these two cases apart: "not triggered" means the measurement itself ruled the experiment out; this outcome means the experiment was authorised by measurement and a human chose not to spend the cost anyway.
- **Verbatim human decision: "DECLINE"** — presented with the fired trigger, the projected build cost, the governing prior, and the structural caveat below, answered via the Task 1 blocking-human checkpoint of plan 10-12.
- **Projected build cost, computed live (not assumed) at decision time:** full `config.SEASONS` scope (`data/transfermarkt.py`'s own "full authorised scope" precedent) — 244,425 `(season, gw, player_code)` windows -> **~14.1 days** of continuous GDELT fetching at the locked `_GDELT_MIN_INTERVAL_S = 5.0` throttle; the narrower 6-season measurement scope — 156,075 windows -> **~9.0 days**. Either bound is before any Guardian pass or retry/backoff overhead.
- **Guardian never ran; GDELT-only mode was never reached either** — no measurement was taken at all on the DECLINE route. `data/news.py` was never created, `config.NEWS_COLS` was never registered.
- **Governing prior cited as the relevant prior evidence:** `danielfrees/mlpremier` (arXiv 2405.02412) — the paper the source todo cites to mine for a news-sentiment approach — published a **negative** result for Guardian-based news sentiment, underperforming both its own CNN and its Ridge/LightGBM baselines in that paper's own reported comparison.
